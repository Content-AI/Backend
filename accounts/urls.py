...
from .views import generate_new_api_key,get_api_key,why_subscribe,create_tickets,google_update,linkedin,visitor_data,track_visitor,total_account,users_data,GeneralSettingViewSet,CookieTokenRefreshView,GoogleSocialAuthView,FacebookSocialAuthView, CookieTokenObtainPairView # Import the above views

from accounts.views import VisitorDataApiView
from accounts.views import AggregatedVisitorStats


router = DefaultRouter()
router.register('user', views.RegisterView,basename='register_acc')
router.register('general-setting', views.GeneralSettingViewSet,basename='GeneralSetting')
...
urlpatterns = [    
    ...
    path('accounts_data/login_using_token/', views.generate_otp_by_email, name='generate_otp_by_email'),
    ...
    path('grouped-visitor-data/', VisitorDataApiView.as_view(), name='visitor_data_api'),
    path('aggregated-visitor-stats/', AggregatedVisitorStats.as_view(), name='aggregated-visitor-stats'),
    
    path('create-tickets/', views.create_tickets, name='create-tickets'),
    path('why-subscribe/', views.why_subscribe, name='why_subscribe'),
    
    
    path('generate_new_api_key/', views.generate_new_api_key, name='generate_new_api_key'),
